simulate_data <- function(n){
	...
	flog.debug("Q values: %s", q_values[1:10], capture=T) 
	flog.debug("P values: %s", p_values[1:10], capture=T) 
	# pvalues cannot be exactly 0
	flog.debug("min/max p: %s, %s", min(p_values), max(p_values)) 
	flog.debug("min/max q: %s, %s", min(q_values), max(q_values)) 
